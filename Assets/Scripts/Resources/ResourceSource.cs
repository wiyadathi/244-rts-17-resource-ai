...
public class ResourceSource : MonoBehaviour
{
    ...
    [SerializeField] private int maxQuantity; //จำนวนสูงสุดของ resource ที่มีในเกม เช่น มีต้นไม้ 100 ต้น ถูกตัดไปแล้วเท่าไหร่  0 คือหมด ต้นไม้ก็หายไป

    [SerializeField] private GameObject selectionVisual; //ring แสดงเมื่อเลือกต้นไม้
    ...
    public void GatherResource(int amountRequest)
    {
        ...
        if (amountRequest > quantity) //not enough ของที่มีไม่พอ
        ...
        if (quantity <= 0)    //ของหมด ทำลายทิ้ง
        {
            Destroy(gameObject);
        }
    }

    // toggles green selection ring around resource  เปิดปิดวงแหวน
